...
def _resolve_encounter_warehouse(doc) -> str:
    """
    Decide warehouse based on Encounter Place + User Role.
    """
    ...
    if "Administrator" in roles or "System Manager" in roles:
        if place == "opd":
            return "OPD Warehouse - SR"
        return "Packaging Warehouse - SR"
...
def validate_encounter_workflow(doc, method):
    ...
    # -------------------------------------------------
    # HARD STOP: No changes once terminal
    # -------------------------------------------------
    if db.get("sr_encounter_status") in TERMINAL_STATES:
    ...
    if "Agent" in roles:
        ...
        if not doc.is_new() and db.get("sr_encounter_status") != "Draft":
    ...
    if "Payment Approver" in roles:
        if db.get("sr_encounter_status") != "Payment Approvel Requested":
            frappe.throw("Payment Approver can act only at Payment Approvel Requested")
    ...
    if "Doctor PRX" in roles:
        if db.get("sr_encounter_status") != "PRX Requested":
    ...
    if "Packaging Biller" in roles:
        if db.get("sr_encounter_status") != "Ready to Dispatch":
