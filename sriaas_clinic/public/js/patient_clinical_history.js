  ...
  refresh(frm) {
    ...
    frm.add_custom_button("🖨️ Print History", async () => {
      try {
        ...
        const hasNotes = (e) => {
          return [e.sr_complaints, e.sr_observations, e.sr_investigations, e.sr_notes]
            .some(v => clean(v).length > 0);
        };

        // 1) Get current patient details
        ...
        // 3) Filter to only rows with clinical notes
        const rows = allRows.filter(hasNotes);
        ...
        if (!rows.length) {
          const html = `${css}${header}<p class='muted'>No encounters with Clinical Notes found for this patient.</p>`;
          const w = window.open("", "_blank");
          w.document.write(`<html><head><title>Patient Clinical History</title></head><body>${html}</body></html>`);
          w.document.close();
          return;
        }
        ...
        console.error("Print Full History error:", err);
